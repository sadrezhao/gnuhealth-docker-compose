...
python3 ${GNUHEALTH_DIR}/tryton/server/${TRYTOND}/bin/trytond-admin -c ${GNUHEALTH_DIR}/tryton/server/config/trytond.conf -d ${GHDATABASE} --activate-dependencies -u health health_lab health_surgery health_radiology

# Activate modules in demo database
echo Activating GNUHealth modules for database ghdemo44
python3 ${GNUHEALTH_DIR}/tryton/server/${TRYTOND}/bin/trytond-admin -c ${GNUHEALTH_DIR}/tryton/server/config/trytond.conf -d ghdemo44 --activate-dependencies -u health_radiology
